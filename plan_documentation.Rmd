...
```{r}
matching.plot <- function(match.data, xvar, yvar, tvar, reverse=F){
  ...
  if (reverse) m.data$ord <- max(m.data$ord) - m.data$ord + 1
  ...
  gp <- ggplot(d, aes(x=x, y=y, group=subclass, frame=rframe)) +
    geom_point(aes(color=t, shape=t), alpha=0.5, size=3) +
    geom_line() +
...
#matching.plot(prop.matches, "age", "educ", "treat", reverse=T)
...
combined.plot <- function(f, d, ord, rep, xvar, yvar, outcome){ 
  ...
  y.range <- c(min(s.means.cum[c('full.smd.x', 'matched.smd.x', 
                                 'full.smd.y', 'matched.smd.y')]), 
               max(s.means.cum[c('full.smd.x', 'matched.smd.x', 
                                 'full.smd.y', 'matched.smd.y')]))
  ...
  gg3 <- ggplotly(gg3) %>%
    animation_opts(transition = 0, redraw=T, frame=400) %>% 
      add_lines(data=s.means.cum, x=~n, 
                y=~full.smd.x, inherit = F, name=xvar
                # ,
      ...
      add_lines(data=s.means.cum, x=~n, 
                y=~full.smd.y, inherit = F, name=yvar) %>%
      layout(showlegend = FALSE,
             xaxis = axis.settings("n observations"),
             yaxis = axis.settings("Standarised Mean Difference", y.range, F)
             )
  ...
  agg.ests <- get.estimates(m.matches, outcome)
  
  y.range <- c(min(agg.ests[c('estimate', 'raw', 'weighted', 'stratified'),]),
               max(agg.ests[c('estimate', 'raw', 'weighted', 'stratified'),])
               )
  ...
  gg4 <- (ggplot(comb.match.ests.cum, 
      ...
      layout(showlegend = FALSE,
             xaxis = axis.settings("n observations"),
             yaxis = axis.settings("Estimate of Treatment Effect")
             )
  
    subplot(gg1, gg2, gg3, gg4, 
            widths = c(0.47,0.47), heights = c(0.47,0.47),
            margin = 0.03, shareX = F, shareY = F, nrows = 2,
            titleX = T, titleY = T) %>% 
      # layout(slider=list()) %>%
      animation_opts(transition = 0, redraw=T, frame=400) %>%
      animation_slider(currentvalue = list(prefix = "Number of obs removed: ",
                                           ...
                                           font = list(color="black"))) 
    # %>%
    #   animation_slider(steps=list(method="update"))
...
((c(1,10)[2]-c(1,10)[1])*.1)*c(-1,1)+c(1,10)
...
p <- combined.plot(f, d, ord, rep, xvar, yvar, y) %>% plotly_build()
saveRDS(p, "inst/app/www/plot.rds")
...
sim.data2 <- function(
  ...
  df <- data.frame(MASS::mvrnorm(n, mu = means, Sigma = varcovarMat)) %>%
    mutate(t = rbinom(n, 1, prob = arm::invlogit(weight_t1*X1 + weight_t2*X2)),
           y = weight_y0 + te*t + weight_y1*X1 + weight_y2*X2)# + noise)

  #if (table(df$t)[1] < 50) df$t <- ifelse(df$t==0,1,0) 
...
ggplot(df, aes(X1,X2, color=t)) + geom_point()


combined.plot(f=t~X1+X2, d=df, ord="data", rep=T, xvar="X1", yvar="X2", outcome="y")
